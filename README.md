...
> Working title "CTXAI" — rename freely.
...
- **Direct provider APIs** — no aggregator in between:
  ...
  - Seedream 4.0 via BytePlus ModelArk (synchronous image generation)
...
```bash
git clone https://github.com/<you>/ctxai && cd ctxai
cat > .env <<EOF
...
| --- | --- | --- |
| Kling | text-to-video, image-to-video | [Kling API docs](https://app.klingai.com/global/dev/document-api) → API keys (Access Key + Secret Key) |
